...
export class FrequencyConstroller {
  ...
  @ApiOperation({
    ...
    description:
      "Retrieves a list of all classes a specific user is assigned to.",
  })
  @ApiParam({
    name: "userId",
    type: "number",
  ...
  @ApiOperation({
    ...
    description:
      "Retrieves the consolidated attendance information for all active students on a given date, showing their attendance status and eligibility for general attendance tracking",
  })
  @ApiQuery({
    name: "date",
    required: true,
    ...
    example: "2025-09-20",
    description:
      "The date for which to retrieve attendance records (YYYY-MM-DD format)",
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    ...
    description:
      "Updates the general attendance status for multiple students. Present students will have records created or updated, while absent students will have records removed from the frequency table.",
  })
  @ApiBody({
  ...
  async updateGeneralAttendance(
    @Body() updateDto: UpdateGeneralAttendanceRequestDTO
  ): Promise<void> {
  ...
  @ApiOperation({
    ...
    description:
      "Retrieves the attendance records for all students in a specific class on a given date, including attendance percentages and statuses.",
  })
  @ApiQuery({
    name: "classId",
    type: Number,
  ...
  @ApiResponse({
    status: 400,
    description:
      "Invalid request parameters (classId must be a number, date must be a valid date)",
  })
  @ApiResponse({
    status: 500,
    description: "An unexpected internal server error occurred",
  ...
  async getClassAttendanceList(
    ...
    return await this.frequencyService.getAttendanceListByClassAndDate(
      date,
      classId
    );
  }
  ...
  @ApiOperation({
    ...
    description:
      "Creates attendance records for all enrolled students in the specified class, setting their default status to PRESENTE.",
  })
  @ApiBody({
  ...
  @ApiResponse({
    status: 404,
    description:
      "The class has no enrolled students or the class was not found",
  })
  @ApiResponse({
    status: 500,
  ...
  async postClassAttendance(
    ...
    return await this.frequencyService.createAttendanceList(
      body.date,
      body.classId
    );
  }
  ...
  @ApiOperation({
    ...
    description:
      "Updates the attendance status and notes for multiple students in a specific class on a given date.",
  })
  @ApiBody({
    ...
    schema: {
      example: {
        turmas: [
          {
            id: 10,
            name: "Turma de Música Iniciante",
            state: "ATIVA",
            activity: { id: 1, name: "Música" },
            level: { id: 2, name: "Iniciante" },
            students: [{ id: 5, fullName: "Aluno 1" }],
            teachers: [{ id: 101, fullName: "Maria Silva" }],
          },
        ],
      },
    },
