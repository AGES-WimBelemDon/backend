...
export class FrequencyDTOMapper {
  ...
  static toStudentGeneralAttendanceDTO(
    prismaClass: PrismaStudentGeneralFrequency,
  ): StudentGeneralAttendanceResponseDTO {
    const studentRef = {
      studentId: prismaClass.studentid,
      fullName: prismaClass.fullname,
      generalAttendanceAllowed:
        prismaClass.generalattendanceallowed === "true" ? true : false,
      status: prismaClass.status,
    };
    return studentRef;
  }
  static toStudentClassAttendanceItemDTO(
    result: PrismaStudentClassAttendance,
  ): StudentClassAttendanceItemDTO {
